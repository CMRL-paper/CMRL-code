...
def load_data(file_path, encoder):
    code_pair_list = []
    ...
    for item in data:
        ...
        for code_pair in item_code:
            ...
            if len(PLcode) < 7 or len(ASMcode) == 0:     # 去除长度小于7的代码片段
                ...
                ASMcode_embedding = encoder.encode(ASMcode)
                code_pair_list.append([ASMcode_embedding, PLcode])
            # print(PLcode,'\n',ASMcode)
            # print('--------------------')
        # print('\n==================\n')
...
def genDataSet():
    ...
    #file_path = ['./jsonDataSet/json-newcurlasm.txt']
    ...
    for path in file_path:
        code_pair_list_tmp = load_data(path, palmtree)
